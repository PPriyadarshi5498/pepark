...
const ChatLabel = ({openMenu, setOpenMenu, id, name}) => {
  ...
  const deleteHandler = async () =>{
    try {
      ...
      if(!confirm) return
      const {data} = await axios.post('/api/chat/deleta', {chatId: id} )
      if(data.success){
        fetchUsersChats()
        setOpenMenu({id: 0, open: false})
        toast.success(data.message)
  ...
  return (
    <div onClick={selectChat} className='flex items-center justify-between p-2
    text-white/80 hover:bg-white/10 rounded-lg text-sm group cursor-pointer'>
      
      <p className='group-hover:max-w-[83.33%] truncate'>{name}</p>
      ...
      aspect-square hover:bg-black/80 rounded-lg'>
        ...
        <Image src={assets.three_dots} alt='' className={`w-4 ${openMenu.id === id && openMenu.open ? ''
        : 'hidden'}group-hover:block`} />

        <div className={`absolute ${openMenu.id === id && openMenu.open ? 'block' : 'hidden'} -right-36 top-6 bg-gray-700 rounded-xl w-max p-2 z-10 hidden group-hover:block`}>
